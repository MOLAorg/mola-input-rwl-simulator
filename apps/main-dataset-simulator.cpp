...
// Aux funcs:
string completePath(const string &abs_or_relative_path,
                    const string &script_base_filename);

double graph_edge_weight(const RWT_adjacency_graph &graph,
                         const TNodeID id_from, const TNodeID id_to,
                         const RWT_adjacency_graph::edge_t &edge);
...
int main(int argc, char **argv) {
  try {
    ...
    // Parse cfg file:
    CConfigFile cfg(sCfgFil);

    // Get input RWL file:
    string sFil =
        cfg.read_string("world", "input", "", true /* fail if not found*/);
    sFil = completePath(sFil, sCfgFil);

    // Compile & run the "world" model ----------------------------
    const int random_seed =
        cfg.read_int("world", "random_seed", -1 /* default val */);
    if (random_seed < 0)
      rng.randomize();
    else
    ...
    if (show_live_3D) {
      win3D = mrpt::gui::CDisplayWindow3D::Create("RWL compilation result", 640,
                                                  480);
    ...
    // 2) A sequence of node IDs, corresponding to the 0-based indices of the
    // RWL
    //      program "NODE" primitives:
    //  source_node_IDs = ID_1 ID_2 ID_3 .... ID_N
    const string sSourceNodeIDs =
        cfg.read_string("path", "source_node_IDs", "");
    const string sSourceNodePathIDs =
        cfg.read_string("path", "source_node_path_IDs", "");
    ...
    if (!sSourceNodeIDs.empty()) {
      ...
      ASSERTMSG_(!lst.empty(),
                 "[path].source_node_path_IDs: Expected list of IDs!");
      ...
      for (size_t i = 0; i < lst.size(); i++) {
        ...
        if (mrpt::system::strCmpI(lst[i], "RANDOM")) {
          size_t idx;
          rng.drawUniformUnsignedIntRange(idx, /*min/max:*/ 0,
                                          the_world.nodes.size() - 1);
          lst_idxs.push_back(idx);
        } else if (mrpt::system::strStartsI(lst[i], "RANDOM*")) {
          ASSERTMSG_(lst[i].size() > 7, "RANDOM*# expects # of random nodes!");

          const size_t nRndNodes =
              static_cast<size_t>(rwt::str2num(lst[i].substr(7)));
        ...
        for (size_t j = 0; j < lst_idxs.size(); j++) {
          ...
          if (prev_idx == INVALID_NODEID) {
            ...
            // Create a spanning tree with Dijkstra to find out the path
            // between: prev_idx -> idx.
            const mrpt::graphs::CDijkstra<RWT_adjacency_graph> tree(
                the_world.graph, prev_idx, &graph_edge_weight);

            // Get path:
            mrpt::graphs::CDijkstra<RWT_adjacency_graph>::edge_list_t lst_edges;
            tree.getShortestPathTo(idx, lst_edges);

            ASSERTMSG_(!lst_edges.empty(),
                       mrpt::format(
                           "ERROR: No topological path found between %u <-> %u",
                           static_cast<unsigned int>(prev_idx),
                           static_cast<unsigned int>(idx)));

            for (mrpt::graphs::CDijkstra<
                     RWT_adjacency_graph>::edge_list_t::const_iterator it =
                     lst_edges.begin();
                 it != lst_edges.end(); ++it) {
              const TPairNodeIDs pair_ids = *it;
              const TNodeID next_id =
                  pair_ids.first == prev_idx ? pair_ids.second : pair_ids.first;
    ...
    cout << "[3<] Path generated OK: " << simulation_waypoints.size()
         << " waypoints.\n";
    cout.flush();

    // load the rest of options:
    // ------------------------------
    const string sOutFilesPrefix =
        cfg.read_string("dataset-format", "output_files_prefix", "OUT_",
                        true /* fail if not present */);

    RWT_PathOptions pathParams;
    pathParams.max_step_lin = cfg.read_double("path", "max_step_lin", 0.10);
    pathParams.max_step_ang =
        mrpt::DEG2RAD(cfg.read_double("path", "max_step_ang", 10));

    RWT_SensorOptions sensorParams(cfg);
    sensorParams.sOutFilesPrefix = sOutFilesPrefix;
    sensorParams.observations_as_c_structs = sensorParams.cfg_file.read_bool(
        "dataset-format", "observations_as_c_structs", false);
    const string comment_char =
        (sensorParams.observations_as_c_structs ? "//" : "%");

    RWT_OutputOptions outputParams;
    outputParams.win3D = win3D;
    outputParams.show_live_3D = (win3D.get() != nullptr);
    outputParams.show_live_3D_decimate = cfg.read_int(
        "path", "show_live_3D_decimate", outputParams.show_live_3D_decimate);
    outputParams.show_live_3D_sleep_ms = cfg.read_int(
        "path", "show_live_3D_sleep_ms", outputParams.show_live_3D_sleep_ms);

    if (outputParams.is_binary) {
      THROW_EXCEPTION("TO DO");
    } else {
      cout << "Saving map ground truth and preparing result files...";
      cout.flush();

      const string sOutSensor =
          sOutFilesPrefix + (sensorParams.observations_as_c_structs
                                 ? string("_SENSOR.c")
                                 : string("_SENSOR.txt"));
      outputParams.output_text_sensor.open(sOutSensor.c_str());
      ASSERTMSG_(
          outputParams.output_text_sensor.is_open(),
          mrpt::format("Couldn't open output file: '%s'", sOutSensor.c_str()));
      ...
      ASSERTMSG_(
          outputParams.output_text_groundtruth.is_open(),
          mrpt::format("Couldn't open output file: '%s'", sOutGT.c_str()));
      ...
      ASSERTMSG_(
          fOutGTMap.is_open(),
          mrpt::format("Couldn't open output file: '%s'", sOutGTMap.c_str()));
      fOutGTMap << "% Landmark ground truth global positions (first row is for "
                   "LM index=0, next is 1 and so on)\n"
                   "%    X             Y              Z        \n"
                   "% -----------------------------------------\n";

      const string sOutWayPointsMatlab =
          sOutFilesPrefix + string("_DRAW_WAYPOINTS.m");
      std::ofstream fOutWayPointsMatlab(sOutWayPointsMatlab.c_str());
      ASSERTMSG_(fOutWayPointsMatlab.is_open(),
                 mrpt::format("Couldn't open output file: '%s'",
                              sOutWayPointsMatlab.c_str()));

      RWT_SaveMatlabOptions matlab_opts;
      matlab_opts.nodes_size = 4;
      rwt::save_rwt_as_matlab_script(the_world, fOutWayPointsMatlab,
                                     matlab_opts);

      const auto &lm_xs = the_world.landmarks.getPointsBufferRef_x();
      const auto &lm_ys = the_world.landmarks.getPointsBufferRef_y();
      const auto &lm_zs = the_world.landmarks.getPointsBufferRef_z();
    ...
    const int random_seed_sensor =
        cfg.read_int("sensor", "random_seed", -1 /* default val */);
    if (random_seed_sensor < 0)
      rng.randomize();
    else
      rng.randomize(random_seed_sensor);

    simulate_rwt_dataset(simulation_waypoints, the_world, pathParams,
                         sensorParams, outputParams);
