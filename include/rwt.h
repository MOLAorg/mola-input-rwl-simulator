...
using mrpt::config::CConfigFileBase;
using mrpt::containers::ci_less;
using namespace mrpt::img;
#else
#include <mrpt/utils/CConfigFileBase.h>
#include <mrpt/utils/ci_less.h>
using mrpt::utils::CConfigFileBase;
using mrpt::utils::ci_less;
namespace mrpt {
using RAD2DEG = mrpt::utils::RAD2DEG;
using DEG2RAD = mrpt::utils::DEG2RAD;
using square = mrpt::utils::square;
} // namespace mrpt
#endif
...
typedef std::map<std::string, RWT_List, ci_less>
    TListSet; //!< A set of RWT_Lists, indexed by name (case insensitive)
...
struct RWT_World {
  ...
  mrpt::maps::CSimplePointsMap landmarks; //!< XYZ coordinates of all landmarks,
                                          //!< accesible thru a KD-tree
  mrpt::maps::CSimplePointsMap
      nodes; //!< XYZ coordinates of all nodes, accesible thru a KD-tree
  RWT_adjacency_graph graph; //!< graph.edges contain the existing paths
                             //!< between \a nodes above
};
...
struct WRL_RenderOptions {
  ...
  struct CornerParams {
    ...
    CornerParams(double _scale, double _line_width = 1.0)
        : scale(_scale), line_width(_line_width) {}
    double scale;
    double line_width;
  };
  ...
  /** Ctor */
  WRL_RenderOptions()
      : show_world_origin_corner(true), world_origin_corner_params(1., 3.) {}
...
struct RWT_SaveMatlabOptions {
  RWT_SaveMatlabOptions() : nodes_size(7.0), edges_width(1.0) {}
...
struct RWT_PathOptions {
  RWT_PathOptions() : max_step_lin(0.1), max_step_ang(mrpt::DEG2RAD(10)) {}
...
struct RWT_SensorOptions {
  RWT_SensorOptions(CConfigFileBase &_cfg_file)
      : cfg_file(_cfg_file), observations_as_c_structs(false) {}

  /** Source of extra parameters for loading upon construction */
  CConfigFileBase &cfg_file;
  std::string sOutFilesPrefix; //!< For dumping extra sets of parameters used
                               //!< in simulation, for future reference.
...
struct RWT_OutputOptions {
  RWT_OutputOptions()
      : win3D(), show_live_3D(false), show_live_3D_decimate(1),
        show_live_3D_sleep_ms(10), is_binary(false) {}

  mrpt::gui::CDisplayWindow3D::Ptr
      win3D; //!< If not an empty smart pointer, the
             //!< window to show simulation live.
  ...
  // The following file streams must be opened by the caller to \a
  // simulate_rwt_dataset
...
/** Compile an input RWT file into a program and run it to generate the
 * synthetic world. This function is equivalent to successive calls to \a
 * compile_rwt_program() and \a run_rwt_program(), plus the possibility of
 * caching the generated world in cache files to avoid running exactly the
 * same program more than once.
 *
 *  \return false on any error, and dump info to std::cerr
 */
bool compile_and_run_rwt_program(const std::string &file,
                                 RWT_Program &out_program, RWT_World &out_world,
                                 bool enable_file_caching = true);
 ...
 * These opengl objects are created with names for allowing the user to
 * manipulate them:
 *   - "node_labels": The text labels of all node IDs. It's initially
 * invisible, get it and call setVisibility(true) to show.
 *   - "landmarks": All landmarks as a point cloud.
 */
void world_to_opengl(const RWT_World &world,
                     mrpt::opengl::CSetOfObjects &out_gl,
                     const WRL_RenderOptions &renderOpts = WRL_RenderOptions());

/** Load a world from a compiled .crwt.gz file \sa
 * compile_and_run_rwt_program, save_rwt_world If a source file (.rwt) is
 * provided in \a source_file, its HASH will be compared with that within the
 * compiled file to assure that the program hasn't changed since compilation,
 * returning false otherwise. \return false on any error  */
bool load_rwt_world(RWT_World &world, const std::string &crwt_file,
                    const std::string &source_file = std::string());

/** Save a world to a compiled .crwt.gz file \sa compile_and_run_rwt_program,
 * load_rwt_world If \a source_file is provided, a HASH will be saved (see \a
 * load_rwt_world() for its utility). \return false on any error  */
bool save_rwt_world(const RWT_World &world, const std::string &file,
                    const std::string &source_file = std::string());

/** Simulate an arbitrarily complex robot path (given an input sequence of
 * waypoints), a world model and a set of sensor and output parameters.
 */
void simulate_rwt_dataset(const std::vector<mrpt::math::TPoint3D> waypoints,
...
void save_rwt_as_matlab_script(const RWT_World &world, std::ostream &o,
