...
/** Sensor implementation: A generic sensor giving (range, yaw, pitch)
 * measurements.
 */
struct SensorSimul_RangeBearing : public SensorSimulBase {
  ...
  SensorSimul_RangeBearing(const RWT_World &world,
                           const RWT_SensorOptions &sensorParams)
      : SensorSimulBase(world, sensorParams), m_minRange(0), m_maxRange(20),
        m_fov_h(mrpt::DEG2RAD(180)), m_fov_v(mrpt::DEG2RAD(120)),
        m_sensor_pose_on_robot(0, 0, 0, mrpt::DEG2RAD(0), mrpt::DEG2RAD(0),
                               mrpt::DEG2RAD(0)),
    ...
    m_fov_h = mrpt::DEG2RAD(sensorParams.cfg_file.read_double(
        "sensor", "fov_h", mrpt::RAD2DEG(m_fov_h)));
    m_fov_v = mrpt::DEG2RAD(sensorParams.cfg_file.read_double(
        "sensor", "fov_v", mrpt::RAD2DEG(m_fov_v)));

    m_range_noise_std = sensorParams.cfg_file.read_double(
        "sensor", "range_noise_std", m_range_noise_std);
    m_yaw_noise_std = mrpt::DEG2RAD(sensorParams.cfg_file.read_double(
        "sensor", "yaw_noise_std", mrpt::RAD2DEG(m_yaw_noise_std)));
    m_pitch_noise_std = mrpt::DEG2RAD(sensorParams.cfg_file.read_double(
        "sensor", "pitch_noise_std", mrpt::RAD2DEG(m_pitch_noise_std)));
    ...
    m_sensor_pose_on_robot.fromString(sensorParams.cfg_file.read_string(
        "sensor", "sensor_pose_on_robot", m_sensor_pose_on_robot.asString()));
    m_sensor_pose_on_robot.getAsVectorVal().transpose().saveToTextFile(
  ...
  virtual void simulate(SimulContext &sim, const bool is_binary,
    ...
    for (size_t i = 0; i < nearby_LMs.size(); i++) {
      ...
      if (range < m_minRange || range > m_maxRange || std::abs(yaw) > m_fov_h ||
          std::abs(pitch) > m_fov_v)
        continue;

      // Add noise:
      range += rng.drawGaussian1D(0, m_range_noise_std);
      yaw += rng.drawGaussian1D(0, m_yaw_noise_std);
      pitch += rng.drawGaussian1D(0, m_pitch_noise_std);
      ...
      lst_observed_landmarks.push_back(
          std::pair<size_t, TrangeBearingSensorObsData>(idxLM, obs));
    }
  ...
  virtual void describeObservation(RWT_OutputOptions &outputParams) {
    outputParams.output_text_sensor << "%                           RANGE(m)  "
                                       "YAW(rad)  PITCH(rad)           \n"
                                       "% "
                                       "---------------------------------------"
                                       "----------------------------\n";
