...
struct RWT_run_context {
  struct StackableContext {
    mrpt::math::TPose3D
        cursor; //!< SE(3) poses with rotations parameterized as yaw,pitch,roll
    size_t current_node;
  };
  ...
  RWT_run_context(const RWT_Program &_program, RWT_World &_world)
      : current_node(0), cursor(), program(_program), out_world(_world) {
    out_world.clear();
    out_world.nodes.insertPoint(0, 0,
                                0); // The first node is always at the origin!
  }

  void push_context() {
    StackableContext d;
#if MRPT_VERSION >= 0x199
    d.cursor = this->cursor.asTPose();
#else
  ...
  double eval1st(const RWT_command &cmd) {
    if (cmd.args.empty())
      throw std::runtime_error(
          "*ERROR* Evaluating primitive: Argument expected but none provided.");
    return rwt::str2num(cmd.args[0]);
  }
};

bool recursive_run_rwt_program(const TListSet::const_iterator &it_lst,
                               RWT_run_context &cntx);

bool run_rwt_cmd(const RWT_command &cmd, RWT_run_context &cntx) {
  using mrpt::DEG2RAD;
  ...
  case PRIM_YAW_INC:
    cntx.cursor +=
        mrpt::poses::CPose3D(0, 0, 0, DEG2RAD(cntx.eval1st(cmd)), 0, 0);
    break;
  case PRIM_PITCH_INC:
    cntx.cursor +=
        mrpt::poses::CPose3D(0, 0, 0, 0, DEG2RAD(cntx.eval1st(cmd)), 0);
    break;
  case PRIM_ROLL_INC:
    cntx.cursor +=
        mrpt::poses::CPose3D(0, 0, 0, 0, 0, DEG2RAD(cntx.eval1st(cmd)));
    break;
  ...
  case PRIM_LANDMARK: {
    ...
    cntx.out_world.landmarks.insertPointFast(
        gx, gy, gz); // "Fast" means the KD-tree will not be marked as invalid,
                     // but we don't mind that here.
  ...
  case PRIM_LANDMARK_RANDOM: {
    ...
    // Draw sample:
    const double lx = rng.drawUniform(min_x, max_x);
    const double ly = rng.drawUniform(min_y, max_y);
    const double lz = rng.drawUniform(min_z, max_z);
  ...
  case PRIM_NODE: {
    ...
    if (cmd.args.size() != (is_no_arc ? 1 : 0) &&
        cmd.args.size() != (is_no_arc ? 4 : 3))
      throw std::runtime_error("*ERROR* NODE: None or three arguments expected "
                               "plus optional 'NO_ARC'!\n");
    ...
    if (!cntx.out_world.nodes.empty()) {
      float out_sqr_err;
      revisit_index =
          cntx.out_world.nodes.kdTreeClosestPoint3D(gx, gy, gz, out_sqr_err);
      skip = (out_sqr_err < mrpt::square(1e-1));
    }
    ...
    if (!skip) {
      ...
      // RWT_MESSAGE << "NODE: " << gx << ", " << gy << ", " << gz << " from "
      // << cntx.cursor  << endl;
      cntx.current_node = cntx.out_world.nodes.size() - 1;
    } else {
      // We have revisited an old node:
      // RWT_MESSAGE << "NODE *SKIPPING*: " << gx << ", " << gy << ", " << gz <<
      // " from " << cntx.cursor  << endl;
  ...
  case PRIM_CALL: {
    ...
    if (cmd.args.size() == 2) {
      ...
      if (sNumReps.empty() || sNumReps[0] != '*')
        throw std::runtime_error(
            "*ERROR* CALL: Expected '*N' for number of repetitions!\n");
    ...
    const TListSet::const_iterator it_lst =
        cntx.program.lists.find(callListName);

    if (it_lst == cntx.program.lists.end())
      throw std::runtime_error(mrpt::format(
          "*ERROR* CALL: Unknown list name: '%s'\n", callListName.c_str()));
  ...
  case PRIM_RANDOMIZE: {
    if (cmd.args.size() != 0 && cmd.args.size() != 1)
      throw std::runtime_error(
          "*ERROR* RANDOMIZE: None or one arguments expected!\n");

    if (cmd.args.size() == 1) {
      const double fSeed = rwt::str2num(cmd.args[0]);
      rng.randomize(static_cast<uint32_t>(fSeed));
    } else {
      rng.randomize();
    }
  } break;
