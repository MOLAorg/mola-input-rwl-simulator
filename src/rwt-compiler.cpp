...
using mrpt::io::CTextFileLinesParser;
#else
#include <mrpt/utils/CTextFileLinesParser.h>
using mrpt::utils::CTextFileLinesParser;
#endif
 ...
 */
bool rwt::compile_rwt_program(const std::string &file,
                              RWT_Program &out_program) {
  ...
  try {
    CTextFileLinesParser flp(file);

    flp.enableCommentFilters(false, // filter_MATLAB_comments
    ...
    // The parser state machine is really simple: It can be either within or
    // without
    //  a "LIST" block at any time:
    enum TState { stIdle, stList };
    ...
    while (flp.getNextLine(ss)) {
      ...
      if (strCmpI("LIST", toks[0])) {
        ...
        if (parser_state != stList)
          throw std::runtime_error(mrpt::format(
              "%u: ENDLIST out of LIST block is not allowed\n",
              static_cast<unsigned int>(flp.getCurrentLineNumber())));
        ...
        args.erase(
            args.begin()); // Remove the first one, so the rest are arguments.
        ...
        if (primitive_id == PRIM_INVALID)
          throw std::runtime_error(mrpt::format(
              "%u: No known primitive found in this line:\n Line: '%s'\n",
              static_cast<unsigned int>(flp.getCurrentLineNumber()),
              line.c_str()));
