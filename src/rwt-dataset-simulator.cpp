...
void rwt::simulate_rwt_dataset(
    const std::vector<mrpt::math::TPoint3D> waypoints, const RWT_World &world,
    const RWT_PathOptions &pathParams, const RWT_SensorOptions &sensorParams,
  ...
  simul_path.setInterpolationMethod(mrpt::poses::imSplineSlerp);
#if MRPT_VERSION >= 0x199
  simul_path.setMaxTimeInterpolation(std::chrono::seconds(2));
#else
  simul_path.setMaxTimeInterpolation(2.0);
#endif

  const double t0 = mrpt::system::timestampToDouble(mrpt::system::now());

#if MRPT_VERSION >= 0x199
  const auto At_between_waypoints = std::chrono::seconds(1);
#else
  const mrpt::system::TTimeStamp At_between_waypoints =
      mrpt::system::secondsToTimestamp(1.0); // 1 second
#endif
...

  {
    ...
    mrpt::system::TTimeStamp t = mrpt::system::time_tToTimestamp(t0);
    for (size_t i = 0; i < nWayPoints; i++) {
      ...
      if (!is_last_pt) {
        // Create a SE(3) pose placed at cur_pt and heading towards next_pt with
        // "upwards" looking at +Z:
      ...
      // Trick: For spline interpolator to work we need to add an extra time
      // steps before the beginning and after the end:
      if (i == 0)
        simul_path.insert(t - At_between_waypoints, p);
  ...
  // --------------------------------------
  const string sType =
      sensorParams.cfg_file.read_string("sensor", "type", "", true);
  SensorSimulBase::Ptr sensor;

  if (mrpt::system::strCmpI(sType, "camera"))
    sensor = SensorSimulBase::Ptr(new SensorSimul_Camera(world, sensorParams));
  else if (mrpt::system::strCmpI(sType, "camera_range"))
    sensor =
        SensorSimulBase::Ptr(new SensorSimul_CameraRange(world, sensorParams));
  else if (mrpt::system::strCmpI(sType, "stereo_camera"))
    sensor =
        SensorSimulBase::Ptr(new SensorSimul_CameraStereo(world, sensorParams));
  else if (mrpt::system::strCmpI(sType, "cartesian_sensor"))
    sensor =
        SensorSimulBase::Ptr(new SensorSimul_Cartesian(world, sensorParams));
  else if (mrpt::system::strCmpI(sType, "range_bearing"))
    sensor =
        SensorSimulBase::Ptr(new SensorSimul_RangeBearing(world, sensorParams));
  else if (mrpt::system::strCmpI(sType, "relative_poses"))
    sensor = SensorSimulBase::Ptr(
        new SensorSimul_RelativePoses(world, sensorParams));
  else
    throw std::runtime_error(
        mrpt::format("ERROR: Unknown sensor type: %s", sType.c_str()).c_str());

  // Write human-readable textual description of data output:
  sensor->describeObservation(outputParams);

  // Cache this pointer to avoid looking for it with each
  // iteration.
  mrpt::opengl::CRenderizable::Ptr gl_robot;

  // Approx # steps from user params:
  const size_t nDesiredTimeSteps = 1 + total_path_len / pathParams.max_step_lin;
  const auto t_last =
      mrpt::system::timestampToDouble(simul_path.rbegin()->first);
  const double At_total_tim = t_last - t0;
  const auto At_step = (t_last - t0) / nDesiredTimeSteps;

  cout << "\n"; // start new line for simulation state
  ...
  for (double t = t0; t < (t_last + 1e-4); t += At_step) {
    // Simulate sensor readings at current location:
    // -----------------------------------------------
    mrpt::obs::CObservation::Ptr new_obs_bin;
    std::string new_obs_txt;

    bool valid_interp;
    simul_path.interpolate(mrpt::system::time_tToTimestamp(t), sim.curPose,
                           valid_interp);

    if (!valid_interp) {
      std::cerr << "Invalid interpolation for t=" << t
                << "\r"; // repeat error in the same line
    ...
    if (sensorParams.observations_as_c_structs)
      outputParams.output_text_sensor
          << "my_observation_struct_t  observations_" << sim.step_count
          << "[] = {" << endl;

    mrpt::poses::CPose3DQuat sensor_GT_Pose;
    const mrpt::poses::CPose3DQuat robot_GT_Pose =
        mrpt::poses::CPose3DQuat(sim.curPose);
    // -------------------------------------------
    ...
    if (outputParams.show_live_3D && outputParams.win3D->isOpen()) {
      ...
      if (outputParams.win3D->keyHit()) {
        mrpt::opengl::COpenGLScene::Ptr &scene =
            outputParams.win3D->get3DSceneAndLock();
        ...
        case 'i':
        case 'I': {
          auto obj = mrpt::ptr_cast<mrpt::opengl::CSetOfObjects>::from(
                         scene->getByName("world"))
                         ->getByName("node_labels");
          if (obj)
      ...
      if (++nStepForDecimateGUI >= outputParams.show_live_3D_decimate) {
        ...
        if (!gl_robot) {
          ...
          if (!gl_robot) {
            mrpt::opengl::CSetOfObjects::Ptr gl_rob =
        ...
        outputParams.win3D->setCameraPointingToPoint(
            sim.curPose.x(), sim.curPose.y(), sim.curPose.z());
      ...
      if (do_repaint) {
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(outputParams.show_live_3D_sleep_ms));
      }
    }
...
#if MRPT_VERSION >= 0x199
      auto of =
          mrpt::serialization::archiveFrom(outputParams.output_bin_rawlog);
#else
      auto &of = outputParams.output_bin_rawlog;
#endif
      of << new_obs_bin;
    } else
      outputParams.output_text_sensor << new_obs_txt;

    // GT pose: Save as quaternion since its meaning is clear and unambiguous
    // for everyone:
    outputParams.output_text_groundtruth << mrpt::format(
        "%6u %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n",
        static_cast<unsigned int>(sim.step_count), sensor_GT_Pose.x(),
        sensor_GT_Pose.y(), sensor_GT_Pose.z(), sensor_GT_Pose.quat().r(),
        sensor_GT_Pose.quat().x(), sensor_GT_Pose.quat().y(),
        sensor_GT_Pose.quat().z(), robot_GT_Pose.x(), robot_GT_Pose.y(),
        robot_GT_Pose.z(), robot_GT_Pose.quat().r(), robot_GT_Pose.quat().x(),
        robot_GT_Pose.quat().y(), robot_GT_Pose.quat().z());
    ...
    // Put here so the "continue" in case of not interpol. does not increment
    // it:
    sim.step_count++;
  ...
  if (sensorParams.observations_as_c_structs) {
    ...
    for (size_t i = 0; i < sim.step_count; i++)
      outputParams.output_text_sensor << "{ observations_" << i << ", "
                                      << num_obs_per_timestep[i] << " },\n";
    outputParams.output_text_sensor << "};\n";
  }

  if (sim.warning_no_observation_count)
    cerr << "WARNING: Zero observations were detected by the sensor during "
         << sim.warning_no_observation_count << " frames.\n";

  if (sim.warning_no_interpolation_count)
    cerr << "WARNING: Couldn't interpolate path for "
         << sim.warning_no_interpolation_count
         << " time steps. Some robot move steps may be longer than expected.\n";
}
