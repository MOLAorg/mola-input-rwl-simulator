...
struct SensorSimul_Cartesian : public SensorSimulBase {
  ...
  SensorSimul_Cartesian(const RWT_World &world,
      ...
      : SensorSimulBase(world, sensorParams), m_minRange(0), m_maxRange(20),
        ...
        m_sensor_pose_on_robot(0, 0, 0, mrpt::DEG2RAD(0), mrpt::DEG2RAD(0),
                               mrpt::DEG2RAD(0)),
        m_xyz_noise_std(0), m_check_min_features_per_frame(0) {
    m_minRange =
    ...
    m_xyz_noise_std = sensorParams.cfg_file.read_double(
        "sensor", "xyz_noise_std", m_xyz_noise_std);
  ...
  virtual void simulate(SimulContext &sim, const bool is_binary,
    ...
    for (size_t i = 0; i < nearby_LMs.size(); i++) {
      ...
      if (dist < m_minRange || dist > m_maxRange || std::abs(yaw) > m_fov_h ||
          std::abs(pitch) > m_fov_v)
        continue;

      // Add noise:
      lx += rng.drawGaussian1D(0, m_xyz_noise_std);
      ly += rng.drawGaussian1D(0, m_xyz_noise_std);
      lz += rng.drawGaussian1D(0, m_xyz_noise_std);

      TCartesianSensorObsData obs;
      obs.pt = TPoint3D(lx, ly, lz);
      lst_observed_landmarks.push_back(
          std::pair<size_t, TCartesianSensorObsData>(idxLM, obs));
    }
  ...
  virtual void describeObservation(RWT_OutputOptions &outputParams) {
    outputParams.output_text_sensor << "%                           RELATIVE_X "
                                       " RELATIVE_Y RELATIVE_Z        \n"
                                       "% "
                                       "---------------------------------------"
                                       "----------------------------\n";
  }
