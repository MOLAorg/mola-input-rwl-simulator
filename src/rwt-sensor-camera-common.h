...
namespace rwt
{
	...
	struct SensorSimul_CameraCommon : public SensorSimulBase
	{
				...
				m_is_stereo(is_stereo)
		{
			...
			m_camera_pose_on_robot.fromString( sensorParams.cfg_file.read_string("sensor","sensor_pose_on_robot", m_camera_pose_on_robot.asString() ) );
			...
			m_camera_pose_on_robot.getAsVectorVal().transpose().saveToTextFile(sensorParams.sOutFilesPrefix + string("_POSE_ON_ROBOT.txt"), mrpt::math::MATRIX_FORMAT_FIXED, false, std::string("% Pose of the sensor wrt the robot frame of reference: x (m),y (m),z (m), yaw (rad), pitch (rad),roll (rad)\n") );
		}
