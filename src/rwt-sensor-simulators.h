...
struct SimulContext {
  SimulContext()
      : curPose(), step_count(0), warning_no_observation_count(0),
        warning_no_interpolation_count(0), accum_obs_count(0),
        show_live_3D(false), win3D() {}
  ...
  size_t accum_obs_count; //!< Number of landmakrs observed so far. Can be reset
                          //!< at each iteration to find out how many LMs are
                          //!< seen each timestep
...
struct SensorSimulBase {
  SensorSimulBase(const RWT_World &world, const RWT_SensorOptions &sensorParams)
      : m_world(world), m_sensorParams(sensorParams) {}

  using Ptr = std::unique_ptr<SensorSimulBase>;
  ...
  virtual void simulate(SimulContext &sim, const bool is_binary,
                        ...
                        mrpt::poses::CPose3DQuat &out_GT_sensor_pose) = 0;
